...
const calculateOffsetManually = (timezone, date) => {
    try {
        // Log warning only once for QA visibility
        if (!hasLoggedManualOffsetWarning) {
            console.warn("Timezone offset calculation: Falling back to manual calculation due to limited Intl.DateTimeFormat support");
...
export const getRegionFromTimezone = (timezone) => {
...
const getTimezoneOffsetMinutes = (timezone) => {
    try {
        ...
        const target = new Date(local.toLocaleString("en-US", { timeZone: timezone }));
