 ...
 */
function calculateRequiredSlots(slotTime, durationMinutes) {
...
export const useConvexSlots = (resourceId, eventLength, slotInterval, allDurationOptions, enabled = true) => {
    ...
    const datePresence = useQuery(api.getDatePresence, enabled && selectedDateStr
        ? {
            resourceId,
            date: selectedDateStr,
        }
    ...
    const processedSlots = useMemo(() => {
        ...
        if (datePresence && datePresence.length > 0) {
            const available = formatted.filter((slot) => !hasPresenceConflict(slot.time, eventLength, datePresence, currentUserId));
            const reserved = formatted.filter((slot) => hasPresenceConflict(slot.time, eventLength, datePresence, currentUserId));
    ...
    const isReloading = enabled &&
        selectedDateStr !== null &&
        !processedSlots &&
        prevSlotsRef.current.available.length > 0;
    // Fetch month slots (for calendar dots)
    const fetchMonthSlots = useCallback((currentDate) => {
        if (!enabled)
            return;
