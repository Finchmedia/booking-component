...
function parseTimeStrict(time, field) {
    const match = TIME_RE.exec(time);
    if (!match) {
        throw new Error(`Invalid ${field} "${time}": expected "HH:MM" between 00:00 and 23:59`);
    }
 ...
 */
function assertNonOverlappingWindows(windows, label) {
...
function assertValidWeeklyHours(weeklyHours) {
    const byDay = new Map();
...
export const updateDateOverride = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.customHours !== undefined)
            updates.customHours = args.customHours;
 ...
 */
export async function computeAvailabilityForDate(ctx, scheduleId, date) {
    const schedule = await ctx.db
        .query("schedules")
