 ...
 */
export async function releaseBookingSlots(ctx, resourceId, start, end) {
    ...
    for (const [date, slots] of slotsToFree.entries()) {
        ...
        if (!availability)
            continue;
        const updatedSlots = availability.busySlots.filter((slot) => !slots.includes(slot));
 ...
 */
export async function releaseQuantitySlots(ctx, resourceId, start, end, quantity) {
    const slotsToFree = getRequiredSlots(start, end);
    for (const [date, slots] of slotsToFree.entries()) {
...
//# sourceMappingURL=slot_helpers.js.map