...
export const bookingEmailResultValidator = v.union(v.null(), v.object({
    subject: v.string(),
    html: v.string(),
...
export function assertValidRenderedBookingEmail(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
        throw new Error("BOOKING_EMAIL_RENDERER_INVALID_RESULT");
    }
    const content = value;
    if (Object.keys(content).some((key) => !["subject", "html", "text"].includes(key)) ||
    ...
    const bytes = encoder.encode(content.subject).byteLength +
        ...
        encoder.encode(content.text ?? "").byteLength;
